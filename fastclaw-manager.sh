...
#
# 设计原则（重构后）：
#   - 进程管理（start/stop/restart/status）全部委托给 `bin/fastclaw daemon`，
#     manager.sh 不再维护自己的 PID 文件、不再 nohup tee。
#   - 单一事实来源：PID = ~/.fastclaw/fastclaw.pid，日志 = ~/.fastclaw/logs/gateway.log。
#     这样无论用 `./fastclaw-manager.sh` 还是 `bin/fastclaw daemon` 启动，
#     另一种命令都能正确感知/管理。
...
DAEMON_LOG="${CONFIG_DIR}/logs/gateway.log"   # 与 bin/fastclaw daemon 一致
DAEMON_PID="${CONFIG_DIR}/fastclaw.pid"       # 与 internal/daemon.Paths() 一致
...
check_source_binary() {
    if [ ! -f "${SOURCE_BIN}" ]; then
        error "FastClaw 二进制文件不存在: ${SOURCE_BIN}"
        error "请先运行: $0 build"
        exit 1
    fi
}
...
make_build() {
    ...
    if [ ! -f "${SCRIPT_DIR}/Makefile" ] && [ ! -f "${SCRIPT_DIR}/go.mod" ]; then
        error "未找到构建文件，请确认在 FastClaw 项目目录中"
        exit 1
    fi
    ...
    if [ -f "${SCRIPT_DIR}/Makefile" ]; then
        ...
        ( cd "${SCRIPT_DIR}" && go build -o "${SOURCE_BIN}" ./cmd/fastclaw )
    fi
    if [ -f "${SOURCE_BIN}" ]; then
...
deploy_all() {
    heading "一键部署 FastClaw（build → install → restart）"
    make_build
    install_global
    echo ""
...
# ─── 服务管理（thin wrapper around `bin/fastclaw daemon`）───────────────────
...
check_status() {
    ...
    if [ -f "${CONFIG_DIR}/fastclaw.json" ]; then
        ...
        echo "配置摘要:"
        local store_type
        store_type=$(grep -A1 '"storage"' "${CONFIG_DIR}/fastclaw.json" 2>/dev/null \
...
cleanup_old_logs() {
    ...
    # 以及 <repo>/fastclaw.pid 这个废弃的 PID 文件。
    local stale_pid="${SCRIPT_DIR}/fastclaw.pid"
    ...
    if [ -d "${stale_logdir}" ]; then
        local count
        count=$(ls -1 "${stale_logdir}"/fastclaw-*.log 2>/dev/null | wc -l | tr -d ' ')
        if [ "${count}" != "0" ]; then
            info "移除旧版按日志文件: ${stale_logdir}/fastclaw-*.log (${count} 个)"
            find "${stale_logdir}" -maxdepth 1 -name "fastclaw-*.log" -delete 2>/dev/null || true
        fi
        # 如果目录已空就一并删掉
...
命令:
  install     安装到全局 (~/.local/bin/fastclaw)
  build       重新构建项目 (等价于 make build)
  deploy      一键部署 (build → install → restart)

  start       启动服务 [端口，默认: ${DEFAULT_PORT}]    （委托给 fastclaw daemon start）
  stop        停止服务                              （委托给 fastclaw daemon stop）
  restart     重启服务 [端口，默认: ${DEFAULT_PORT}]    （委托给 fastclaw daemon restart）
  status      查看服务状态 + 端口/配置/日志摘要      （委托给 fastclaw daemon status）
  logs        tail -f 实时日志

  clean       清理历史遗留 (<repo>/fastclaw.pid 和 <repo>/logs/fastclaw-*.log)
