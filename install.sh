#!/bin/sh
# Pawnix Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/softbreezee/claw-os/main/install.sh | sh
# Or:    PAWNIX_INSTALL_DIR=~/bin curl -fsSL ... | sh
set -e

REPO="softbreezee/claw-os"
BINARY="pawnix"
...
choose_install_dir() {
  if [ -n "${PAWNIX_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$PAWNIX_INSTALL_DIR"
  else
...
main() {
  ...
  printf "\n"
  success "Pawnix ${VERSION} installed → ${INSTALL_DIR}/${BINARY}"
  printf "\n"

  if [ "${NEEDS_SOURCE:-0}" = "1" ]; then
    printf "  ${YELLOW}Run this to activate:${NC}\n"
    printf "    source %s\n\n" "$SHELL_RC"
    printf "  Or open a new terminal, then run: ${BOLD}pawnix${NC}\n"
  else
    printf "  Run: ${BOLD}pawnix${NC}\n"
