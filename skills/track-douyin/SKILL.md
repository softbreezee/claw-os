...
name: track-douyin
description: |
  追踪抖音博主更新 + 搜索识别最新内容。触发场景:用户要"盯着某个抖音博主有没有发新
  视频"、"每天看 XX 话题在抖音上的最新内容"、"追踪这几个抖音博主"、设置抖音内容的定时
  监控、搜索抖音并筛选值得看的内容。
---
...
- **登录弹窗**:抖音频繁弹登录引导遮挡内容。snapshot 里若出现登录弹窗元素,
  先尝试关闭(找 close/✕ 的 ref 点掉)再 snapshot;关不掉就用 persistent
  登录态。
- **视频不可读**:再强调一次 —— 抖音追踪只能基于文案+互动数判断,无法理解
  视频内容本身。如果用户要的是"视频里讲了啥",老实告诉他做不到,只能给标题。
